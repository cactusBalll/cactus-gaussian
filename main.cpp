...
int main() {
  ...
  // 测试不同的图像大小
  for (int size = 256 + 37; size <= w; size += 256 + 37) {
    ...
    for (int i = 0; i < size; i++) {
      for (int j = 0; j < size; j++) {
        ...
        // std::cout << out_buffer[i * size + j] << "    " << out_buffer2[i *
        // size + j] << std::endl; if (fabsf(out_buffer[i * size + j] -
        // out_buffer2[i * size + j]) > 1e-4) { 	std::cout << "different result!"
        // << std::endl; 	goto clean;
        // }
